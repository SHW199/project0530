...
public interface UserRepository extends JpaRepository<Customer, String> {
    ...
    //@EntityGraph : 엔티티(객체)들끼리 연관되어 있는 관계가 있으면 그 관계를 그래프로 표현해줄 수 있는 객체 JPA가 어떤 엔티티를 불러올 때 그 엔티티와 연관된 객체를 불러올 것인지에 대한 정보 제공함.
    //getWithRoles 메서드는 Customer 테이블에서 멤버 아이디가 같은 데이터를 조회하는 메서드
