...
public interface ReservationService {
    //원래 interface 객체에서는 메서드를 선언만 하고 {}의 내용은 작성하지 않는 것이 기본 문법인데, interface 객체에서도 {} 내용을 작성해서 사용하려는 메서드는 default 키워드로 선언한다.
    ...
    default Reservation dtoToEntity(ReservationDTO reservationDTO){
        Reservation reservation = Reservation.builder().reservation_id(reservationDTO.getReservation_id()).reservation_day(reservationDTO.getReservation_day()).reservation_num(reservationDTO.getReservation_num())
                .build();
        return reservation;
    }
